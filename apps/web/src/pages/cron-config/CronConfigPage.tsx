...
import { cronConfigApi, type CronConfigItem, type ExecutionLogItem } from '../../api/cron-config'
...
export default function CronConfigPage() {
  ...
  const [logEnv, setLogEnv] = useState<string>(
    (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') ? 'dev' : 'online'
  )
  ...
  useEffect(() => {
    ...
    cronConfigApi.logs(selectedId, logEnv).then(setLogs).catch(() => setLogs([]))
  }, [selectedId, logEnv])

  const save = async () => {
    try {
      if (editing.id) {
        await cronConfigApi.update(editing.id, { name: editing.name, cronExpr: editing.cronExpr, taskType: editing.taskType, taskParams: editing.taskParams ?? undefined, status: editing.status })
      } else {
        const data = { name: editing.name!, cronExpr: editing.cronExpr!, taskType: editing.taskType!, taskParams: editing.taskParams ?? undefined }
  ...
  const handleExecute = async (id: number) => {
    ...
    try {
      ...
      setExecutingIds(prev => { const next = new Set(prev); next.delete(id); return next })
    }
  }

  const envLabel: Record<string, string> = { online: '线上', dev: '开发', all: '全部' }
  ...
  return (
    <div>
      ...
      {/* 错误提示 */}
      {execError && (
        <div style={{ padding: '8px 12px', background: '#fff2f0', border: '1px solid #ffccc7', borderRadius: 4, marginBottom: 12, color: '#ff4d4f' }}>
          {execError}
          <button onClick={() => setExecError(null)} style={{ marginLeft: 12, background: 'none', border: 'none', cursor: 'pointer', color: '#ff4d4f', textDecoration: 'underline' }}>关闭</button>
        </div>
      )}

      {/* 新增按钮 */}
      <div style={{ marginBottom: 16 }}>
        <button onClick={() => { setEditing({}); setIsCustomCron(false); setShowForm(true) }} style={btnStyle}>+ 新增</button>
      </div>

      {/* CRUD 表单 */}
      {showForm && (
        <div style={{ border: '1px solid #d9d9d9', padding: 16, marginBottom: 16, borderRadius: 6 }}>
          <h3 style={{ margin: '0 0 12px' }}>{editing.id ? '编辑' : '新增'}定时任务</h3>
          <input placeholder="任务名称" value={editing.name || ''} onChange={e => setEditing({ ...editing, name: e.target.value })} style={inputStyle} />
          <select
          ...
          <input placeholder="任务类型 (如: feishu_sync)" value={editing.taskType || ''} onChange={e => setEditing({ ...editing, taskType: e.target.value })} style={inputStyle} />
          <textarea placeholder='任务参数 (JSON), 如: {"appToken":"...","tableId":"..."}' value={editing.taskParams || ''} onChange={e => setEditing({ ...editing, taskParams: e.target.value })} style={{ ...inputStyle, minHeight: 60 }} />
      ...
      {/* 配置列表 */}
      <div style={{ border: '1px solid #e0e0e0', borderRadius: 6, overflow: 'hidden', marginBottom: 24 }}>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} style={{
                ...
                <td style={tdStyle}><code>{item.cronExpr}</code></td>
                ...
                <td style={tdStyle}>
                  <button onClick={e => { e.stopPropagation(); setEditing(item); setIsCustomCron(!CRON_PRESETS.some(p => p.value === item.cronExpr)); setShowForm(true) }} style={linkStyle}>编辑</button>
                  <button onClick={e => { e.stopPropagation(); remove(Number(item.id)) }} style={{ ...linkStyle, color: '#ff4d4f', marginLeft: 8 }}>删除</button>
                  <button
                    onClick={e => { e.stopPropagation(); handleExecute(Number(item.id)) }}
            ...
            {items.length === 0 && (
              <tr><td colSpan={7} style={{ ...tdStyle, textAlign: 'center', color: '#999' }}>暂无配置</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div style={{ border: '1px solid #e0e0e0', borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', background: '#fafafa', borderBottom: '1px solid #e0e0e0' }}>
            ...
            <tbody>
              {logs.map(log => (
                <tr key={log.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  <td style={tdStyle}>{new Date(log.startTime).toLocaleString()}</td>
                  <td style={tdStyle}>{log.endTime ? new Date(log.endTime).toLocaleString() : '-'}</td>
                  <td style={tdStyle}>
                    <span style={{
                      ...
                      borderRadius: 4,
                      fontSize: 12,
                      background: log.status === 1 ? '#f6ffed' : log.status === 0 ? '#fff2f0' : '#f5f5f5',
                      color: log.status === 1 ? '#52c41a' : log.status === 0 ? '#ff4d4f' : '#999',
                      border: `1px solid ${log.status === 1 ? '#b7eb8f' : log.status === 0 ? '#ffccc7' : '#d9d9d9'}`,
                    }}>
                      {log.status === 1 ? '成功' : log.status === 0 ? '失败' : '未知'}
                    </span>
                  </td>
                  <td style={tdStyle}>
                    <span style={{
                  ...
                  <td style={{ ...tdStyle, maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', color: log.errorMsg ? '#ff4d4f' : '#999' }}>
                    {log.errorMsg || '-'}
                  </td>
                </tr>
              ))}
