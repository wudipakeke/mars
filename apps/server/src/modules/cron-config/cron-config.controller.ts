...
export class CronConfigController {
  ...
  @Post()
  create(@Body() body: { name: string; cronExpr: string; taskType: string; taskParams?: string }) {
    return this.service.create(body);
  }

  @Put(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body() body: { name?: string; cronExpr?: string; taskType?: string; taskParams?: string; status?: number }) {
